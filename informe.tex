...
depuración de un error. Revisión de código. \\
\bottomrule
\end{tabularx}
...
\begin{itemize}
  ...
  \item \textbf{La ruta \texttt{/403.jpg} se intercepta antes del chequeo de bloqueo.} La imagen es
        un recurso local y no del servidor remoto. Si se evaluara después, el proxy la buscaría
        afuera y obtendría un 404.
  \item \textbf{La imagen se sirve con \texttt{200 OK} y no con \texttt{403}.} El recurso prohibido
        es la página y no la imagen. El 403 corresponde al HTML bloqueado, mientras que la petición
        posterior de \texttt{/403.jpg} es legítima y el proxy la satisface localmente.
\end{itemize}
...
\begin{itemize}
  \item \textbf{No se utiliza \texttt{re}.} El enunciado permite únicamente \texttt{socket},
        \texttt{json} y \texttt{sys}. El método \texttt{replace()} resulta suficiente porque las
        claves del JSON no se solapan entre sí y coinciden en mayúsculas con el contenido de las
        páginas del curso. Un enfoque insensible a mayúsculas habría requerido expresiones
        regulares.
