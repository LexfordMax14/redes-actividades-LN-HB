...
- **`head` es un `dict`**: headers repetidos (ej. dos `Set-Cookie`) se pisan. Limitación asumida, no afecta los casos del enunciado.
...
- **Sin regex** (el enunciado solo permite `socket`, `json`, `sys`). Se pierde `IGNORECASE` y bordes de palabra, pero las claves del JSON coinciden exactamente con el contenido de las páginas del curso.
...
- `http://cc4303.bachmann.cl/secret` → [observación]
- `http://cc4303.bachmann.cl/` → [observación sobre el header]
- `http://cc4303.bachmann.cl/replace` → [observación sobre las palabras]
